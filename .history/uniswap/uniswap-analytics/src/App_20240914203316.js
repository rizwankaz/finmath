...
// Define the query for daily trading volume
const DAILY_VOLUME_QUERY = gql`
  {
    poolDayDatas(first: 14, orderBy: date, orderDirection: desc) {
      date
      volumeUSD
    }
  }
...
function DailyVolume() {
  ...
  // Prepare data for chart
  const dailyVolume = data.poolDayDatas[0];
  const chartData = {
    labels: [new Date(dailyVolume.date * 1000).toLocaleDateString()],
    datasets: [
      {
        label: 'Daily Volume USD',
        data: [parseFloat(dailyVolume.volumeUSD)],
        borderColor: 'rgba(75,192,192,1)',
        backgroundColor: 'rgba(75,192,192,0.2)',
        fill: true,
      },

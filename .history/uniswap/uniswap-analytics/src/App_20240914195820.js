...
const TWENTY_FOUR_HOURS_AGO = Math.floor(Date.now() / 1000) - 24 * 60 * 60; // 24 hours ago in seconds

const SWAP_QUERY = gql`
  query GetRecentSwaps($timestamp: BigInt!) {
    swaps(first: 1000, orderBy: timestamp, orderDirection: desc, where: {timestamp_gt: $timestamp}) {
      id
      amountUSD
      timestamp
    }
...
function Swaps() {
  const { loading, error, data } = useQuery(SWAP_QUERY, {
    variables: { timestamp: TWENTY_FOUR_HOURS_AGO }
  });
